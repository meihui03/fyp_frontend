...
class DatasetVGGFace2(DatasetBase):
    name = 'vggface2test'

    def __init__(self, split: str, transform: Callable[[Tensor], Tensor], cfg: configs.VGGFace2Config):
        assert split in ['train', 'val', 'test']

        dataset = datasets.ImageFolder(cfg.root)

        # Use same total data and class mappings
        total_imgs = dataset.imgs
        class_to_idx = dataset.class_to_idx

        # First split: 80% trainval, 20% test
        trainval_imgs = _split_set(total_imgs, 'train', test_ratio=0.2)
        test_imgs = _split_set(total_imgs, 'test', test_ratio=0.2)

        # Second split: from trainval → 80% train, 20% val
        train_imgs = _split_set(trainval_imgs, 'train', test_ratio=0.2)
        val_imgs = _split_set(trainval_imgs, 'val', test_ratio=0.2)
...
def load_dataset(name, split, preprocess=None, cfg: configs.DatasetConfig = None):
    if cfg is None:
        ...
        cfg = getattr(configs.AllDatasetsConfigs(), name)  # Use an instance
