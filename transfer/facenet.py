...
def finetune_facenet(
    ...
    for epoch in range(epochs):
        ...
        # Save intermediate checkpoint every 2 epochs
        if (epoch + 1) % 2 == 0:
            checkpoint_dir = pathlib.Path(save_dir)
            checkpoint_dir.mkdir(parents=True, exist_ok=True)
            checkpoint_path = checkpoint_dir.joinpath(f'checkpoint_epoch_{epoch+1}.pth')
            torch.save({
                'epoch': epoch + 1,
